...
- **Browser suite:** the seed's category assignment writes only the products whose values differ from the rule's,
  so a second seed run sends the content source nothing and moves no product's updated time.
